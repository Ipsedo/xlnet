...
class WikiDumps(DataProcessor):
    def __init__(self):
        self.file = "./datasets/downloaded/labelised_raw_wiki.txt"
        ex, lbl = self.__parse_file()
    ...
    def get_train_examples(self, data_dir):
        limit = int(self.ratio_train * len(self.example))
        return self.example[:limit]

    def get_dev_examples(self, data_dir):
        limit1 = int(self.ratio_train * len(self.example))
        limit2 = int((self.ratio_train + self.ratio_dev) * len(self.example))
        return self.example[limit1:limit2]

    def get_test_examples(self, data_dir):
        limit1 = int((self.ratio_train + self.ratio_dev) * len(self.example))
        limit2 = int((self.ratio_train + self.ratio_dev + self.ratio_test) * len(self.example))
        return self.example[limit1:limit2]
    ...
    def __parse_file(self):
        ...
        for l in f:
            ...
            inp = InputExample(guid="unused", text_a=txt, text_b=None, label=lbl)
